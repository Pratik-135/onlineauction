...
function countdowntimer(id, time)
{
		...
		if (distance < 0) {
			...
			document.getElementById("countdowntime"+id).innerHTML = "<center><b  style='color: red;'>EXPIRED</b></center>";
                                            ...
                                            <div class="price-box">
<p><b>Actual product cost</b> : NPR <?php echo $rsproduct['product_cost']; ?></p>
<?php 
/*
...
<input type='hidden' name='max_bid_amt' id='max_bid_amt' value='<?php echo $rsproduct['ending_bid']+500; ?>'>
...
<div class="description-content">
	...
	<?php
		...
		else
		{
			...
			{
			echo $rsedit['bidding_id'] . "-" . $rsedit['customer_name'] . " bidded NPR XXXX on " . date("d-M-Y h:i A",strtotime($rsedit['bidding_date_time'])) ."<hr>";
			}
		}
	?>
	</p>
